...
const FormBarangKeluar = (props) => {
  ...
  const formSubmitHandler = async (event) => {
    const komponenMasuk = event.barang;
    try {
      await sendRequest(
        process.env.REACT_APP_BACKEND_URL + "/barang-masuk",
        "PATCH",
        JSON.stringify({
          komponen: komponenMasuk,
        }),
        {
          ...
          {({ values, setFieldValue, errors, handleBlur, touched }) => (
            <Form className="form-barang">
              <Typography className={classes.spacingBiasa} variant="body1">
                Input komponen masuk ke gudang
              </Typography>
